...
export const ShapeOverlay: React.FC<Props> = ({ annotation }) => {
  ...
  const renderShape = () => {
    ...
    switch (annotation.shape) {
      case 'rect': return <rect x={ox} y={oy} width={width} height={height} rx={2} {...props} />
      case 'ellipse': return <ellipse cx={ox + width/2} cy={oy + height/2} rx={width/2} ry={height/2} {...props} />
      case 'line': return <line x1={ox} y1={oy + height} x2={ox + width} y2={oy} {...props} />
      case 'arrow': return (
        <g {...props}>
          <line x1={ox} y1={oy + height} x2={ox + width} y2={oy} />
          <polygon points={`${ox+width},${oy} ${ox+width-10},${oy+8} ${ox+width-10},${oy-8}`} fill={annotation.strokeColor} />
        </g>
      )
      default: return null
    }
